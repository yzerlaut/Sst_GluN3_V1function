...
# %%
folder = '/home/yann.zerlaut/DATA/TADDY/session5/Assembled/'
...
def get_luminosity_summary(data):
    ...
    if 'BlankFirst' in data.metadata['protocol']:                                                                                                              
        ...
        starts = data.nwbfile.stimulus['time_start_realigned'].data[:3]                                                                                      
        tstops = data.nwbfile.stimulus['time_stop_realigned'].data[:3]                                                                                        
        values = ['black' ,'grey', 'dark']                                                                                                                     
...
NR1_FILES, WT_FILES = [], []
for i, protocols in enumerate(DATASET['protocols']):
    if 'ff-gratings-8orientation-2contrasts-15repeats' in protocols:
        FILES.append(DATASET['files'][i])
        if 'NR1' in DATASET['subjects'][i]:
            NR1_FILES.append(DATASET['files'][i])
        else:
            WT_FILES.append(DATASET['files'][i])
...
RESPONSES, FRAC_RESP = {}, {}
for key, FILES in zip(['WT', 'NR1', 'WT_c=0.5'], [WT_FILES, NR1_FILES, WT_FILES]):
    RESPONSES[key], FRAC_RESP[key] = [], []
    for f in FILES:
        data = Data(f, verbose=False)
        responses, frac_resp, shifted_angle = compute_tuning_response_per_cells(data,
                                                protocol_name='ff-gratings-8orientation-2contrasts-15repeats',
                                                contrast=(0.5 if 'c=0.5' in key else 1),
                                                verbose=False)
        RESPONSES[key].append(responses)
        FRAC_RESP[key].append(frac_resp)
...
fig, AX = plot_tunning_summary(RESPONSES['shifted_angle'], FRAC_RESP['WT'], RESPONSES['WT'])
fig, AX = plot_tunning_summary(RESPONSES['shifted_angle'], FRAC_RESP['NR1'], RESPONSES['NR1'])
...
for key, color in zip(['WT', 'NR1', 'WT_c=0.5'],
                      ['k', 'tab:blue', 'tab:orange']):

    RESP= np.concatenate([r for r in RESPONSES[key]])
    # raw
    pt.plot(shifted_angle, np.mean(RESP, axis=0),
            sy=stats.sem(RESP, axis=0), ax=AX[1], color=color)
    AX[1].annotate('mean$\pm$s.e.m. over cells',
            (.6, 1.), va='top', color='grey', xycoords='axes fraction')
    # peak normalized
    N_RESP = [resp/resp[1] for resp in RESP]
    pt.plot(shifted_angle, np.mean(N_RESP, axis=0),
            sy=stats.sem(N_RESP, axis=0), ax=AX[2], color=color)
    AX[2].annotate('mean$\pm$s.e.m. over cells',
            (.6, .8), va='top', color='grey', xycoords='axes fraction')

    # RESP= [np.mean(responses, axis=0) for responses in RESPONSES[key]]
    # # raw
    # pt.plot(shifted_angle, np.mean(RESP, axis=0),
            # sy=np.std(RESP, axis=0), ax=AX[1], color=color)
    # # peak normalized
    # N_RESP = [resp/resp[1] for resp in RESP]
    # pt.plot(shifted_angle, np.mean(N_RESP, axis=0),
            # sy=np.std(N_RESP, axis=0), ax=AX[2], color=color)

    # frac = [np.mean(fr, axis=0) for fr in FRAC_RESP[key]]
    AX[0].bar([len(key)%7], 
            [100*np.mean(FRAC_RESP[key])],
            yerr=[100*np.std(FRAC_RESP[key])], color=color)
    AX[0].annotate((len(key)%4)*'\n'+'%s, N=%i sessions, n=%i cells'%(key,
                            len(FRAC_RESP[key]), np.sum([len(r) for r in RESPONSES[key]])),
            (.6, 1.), va='top', color=color, xycoords='axes fraction')


AX[0].set_ylabel('frac. resp.')
AX[1].set_ylabel('raw $\delta$ $\Delta$F/F')
AX[2].set_ylabel('norm. $\Delta$F/F')
